// Strategy : Frequency based idleness distribution and then shifting
...
vector<Interval> addIdleness(
    ...
    vector<int> density_arr(y, 0);
    sort(all(intervals), [](Interval a, Interval b)->bool{
        if(a.l == b.l) return a.r < b.r;
        return a.l < b.l;
    });
    
    set<Interval, decltype(&endcmp)> active(&endcmp);
    int j = 0; 
    for(int t = 0; t < y; t++){
        while(!active.empty() && active.begin()->r < t){
            active.erase(*active.begin());
        }
        while(j < intervals.size() && intervals[j].l <= t){
            active.insert(intervals[j]); j++;
        }
        density_arr[t] = active.size();
    }
    int window = accumulate(density_arr.begin(), density_arr.begin() + x, 0ll);
    vector<int> density(y, 0);
    density[0] = window;
    for(int i = x; i < y; i++){
        window += density_arr[i] - density_arr[i - x];
        density[i - x + 1] = window;
    }
    ...
    for(int i = 0; i < m; i++){
        int idle_ind = 0;
        for(int j = 0; j <= y - x; j++){
            if(density[j] < density[idle_ind]){
                idle_ind = j;
            }
        }
        ret.pb({idle_ind, idle_ind + x - 1, -1});
        for(int ii = max(0ll, idle_ind - x + 1); ii < idle_ind + x; ii++){
            int overlap = min(idle_ind + x - 1, ii + x - 1) - max(idle_ind, ii) + 1;
            density[ii] = overlap; 
        }
    }
    return ret;
